#include "chap03.h"
#include "game_parts/game_components.h"
...
void printBinaryRepresentation(void *data, size_t size) {
    ...
    for (int i = size - 1; i >= 0; i--) {
        ...
        printf("\n"); // Separate bytes for readability
    }
    printf("\n");
}
...
int main() {
  ...
  while (game_running) {
    ...
    if (current_time - last_time >= tick_interval) {
                    ...
                    if(what_kind_of_spawn == 1){
                        ...
                        if(list_location > 0){
                            ...
                            if (memcmp(&desired_unit, &empty_unit, sizeof(Unit)) == 0) {
                                ...
                                this_players_units->units[this_players_units->unit_count] = spawnee;
      ...
      if(send_message){
          printf("Printing allPlayerUnits:\n");
          for (int i = 0; i < connection_count; i++) {
              PlayerUnits view = *allPlayerUnits[i];
              printBinaryRepresentation(&view, sizeof(view));
          }
